  ...
  if($_POST["action"] == "expense_category_fetch"){
    ...
    while ($row = mysqli_fetch_assoc($expense_category_records)){
      ...
      $data[] = array(
        ...
        "expense_category_status"           =>  $row['expense_category_status'],
  ...
  if($_POST["action"] == "add_expense_category"){
    ...
    if($check_rows > 0) {
      ...
      if(mysqli_query($conn, $sql)){
        $output = array(
          'status'          => 'success',
          'message'         => ' Nouvelle categorie de depense creer avec success.'
        );
      }  
    }
  ...
  if($_POST["action"] == "update_expense_category"){
    ...
    if($check_rows > 0) {
      ...
      if(mysqli_query($conn, $sql)){
        $output = array(
          'status'          => 'success',
          'message'         => ' Les informations sur la catégorie de dépenses ont été mises à jour avec succès.'
        );
      }
    }
    echo json_encode($output);
