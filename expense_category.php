<?php include('include/header.php'); ?>
  ...
  <div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
      <h1 class="h3 mb-0 text-gray-800">Gérer les catégories de dépenses</h1>
      <div>
    ...
    <div class="card shadow mb-4">
      <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Catégories de dépenses</h6>
      </div>
      <div class="card-body">
        ...
        <div class="modal-body">
          <form id="expense_category_name">
            <div class="form-group">
              <label>Catégories de dépenses <i class="text-danger">*</i></label>
              <input type="text" id="expense_category" name="expense_category" class="form-control" maxlength="100" autocomplete="off" placeholder="Entrer Catégories de dépenses">
            ...
            <div class="modal-footer">
              ...
              <button type="button" name="cancel_button" id="cancel_button" class="btn btn-secondary" data-dismiss="modal">Fermer</button>
              <input type="submit" name="save_button" id="save_button" class="btn btn-info" value="Enregistrer" />
            </div>
          </form>
        </div>
      </div>
  ...
  <div class="modal fade" id="readModal">
    <div class="modal-dialog modal-md">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Détails de la catégorie de dépenses</h5>
          <button class="close" data-dismiss="modal">
            <span>&times;</span>
          </button>
        </div>
...
<script>
  ...
  $(document).ready(function(){
    ...
    $('#add_expense_category').click(function(){
      $('#modal_title').text('Ajouter une catégorie de dépenses');
      $('#button_action').val('Save');
      $('#action').val('add_expense_categors');
    ...
    function checkExpenseCategory() {
      if ( $.trim( $('#expense_category').val() ) == '' ){
        $("#expense_category_error_message").html("La catégorie de dépenses est un champ obligatoire.");
    ...
    function checkStatus() {
      if ( $.trim( $('#status').val() ) == '' ){
        $("#status_error_message").html("Le statut est un champ obligatoire.");
    ...
    $('#expense_category_name').on('submit', function(event){
      ...
      if (error_expense_category == false && error_status == false) {
        $.ajax({
          ...
          dataType:"json",
          beforeSend:function(){
            $('#save_button').val('Veuillez Patienter...');
            ...
            if (data.status == 'success') {
              ...
              $("#expense_category_error_message").html("La catégorie de dépenses existe déjàs");
              $("#expense_category_error_message").show();
              $("#expense_category").addClass("is-invalid");
              $('#save_button').val('Save');
              $('#save_button').attr('disabled', false);
            ...
            Notiflix.Notify.Failure('Oops! Quelque chose s est mal passé.');
            $('#save_button').val('Save');
            $('#save_button').attr('disabled', false);
            $('#cancel_button').attr('disabled', false);
          }
    ...
    $(document).on('click', '.update_expense_category', function(){
      ...
      $('#modal_title').text('Mettre à jour la catégorie de dépenses');
